...
if not active_kz:
    # STRICT: no signals outside KZ, just scan log
    try:
...
# SCORING 12 pts - STRICT 8/12
score = 0
score += 2 if nearest['strength'] > 1.5 else 1 # strength
score += 2 if dist < 1.0 else (1 if dist < 2.5 else 0) # proximity
score += 2 if crt == direction else 0 # CRT align
score += 2 if active_kz in ["LONDON","NY_AM","NY_PM"] else 0 # KZ
score += 2 if closes[-1] > np.mean(closes[-20:]) and direction=="BUY" or closes[-1] < np.mean(closes[-20:]) and direction=="SELL" else 1 # trend
score += 2 # structure exists (we found QML)
